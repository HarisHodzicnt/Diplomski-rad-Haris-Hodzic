  ...
  var messagesRef=firebase.database().ref('High Scores');
  messagesRef.on('value', gotData, errData);
  function gotData(data){
    var scores=data.val();
    var changeHighScore=document.getElementById('highScore');
   var highScore=0;
    var key=Object.keys(scores);
    for(var i=0;i<key.length;i++){
      var k=key[i];
     var initials=scores[k]['Simon Game'];
     
     if(initials>highScore)
     {
       highScore=initials;
     }
     changeHighScore.innerHTML=' High Score: '+highScore;
    }
  }
